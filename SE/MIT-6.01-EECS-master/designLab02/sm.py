...
class SM:
    ...
    def start(self,
        ...
        self._SM__debugParams = DebugParams(traceTasks, verbose, compact,
                                            printInput)
    ...
    def step(self, inp):
        ...
        if self._SM__debugParams and self._SM__debugParams.doDebugging:
            if self._SM__debugParams.verbose and not (
                self._SM__debugParams.compact):
                print 'Step:', self._SM__debugParams.k

            self.printDebugInfo(0, self.state, s, inp, o, self._SM__debugParams)
            if self._SM__debugParams.verbose and self._SM__debugParams.compact:
                if self._SM__debugParams.printInput:
                    print 'In:', inp, 'Out:', o, 'Next State:', s
                else:
                    print 'Out:', o, 'Next State:', s

            self._SM__debugParams.k += 1
...
\tabout what is tested.
    ...
    def transduceF(self, inpFn,
        ...
        as the argument. 
        '''
        return _[1]([inpFn(i) for i in range(n)],
                    traceTasks=traceTasks,
                    compact=compact,
                    printInput=printInput,
                    verbose=verbose)
    ...
    def printDebugInfo(self, depth, state, nextState, inp, out, debugParams):
        ...
        if debugParams.verbose and not (debugParams.compact):
            if debugParams.printInput:
                print ' ' * depth, self.name, 'In:', util.prettyString(
                    inp), 'Out:', util.prettyString(
                        out), 'Next State:', util.prettyString(nextState)
            else:
                print ' ' * depth, self.name, 'Out:', util.prettyString(
                    out), 'Next State:', util.prettyString(nextState)
...
\tRaises exception 'InvalidSM' if a problem is found.
        ...
        if inps != None:
            ...
            if type(rv) not in (list, tuple):
                print "[SMCheck] getNextValues provides an invalid return value, '%s'" % repr(
                    rv)
                raise Exception, 'Invalid SM'
            type(rv) not in (list, tuple)
            if not len(rv) == 2:
                print "[SMCheck] getNextValues provides an invalid return value, '%s'" % repr(
                    rv)
                print '[SMCheck] the return value length should be 2, ie (state,output), but it is ', len(
                    rv)
                raise Exception, 'Invalid SM'
            len(rv) == 2
...
class Cascade(SM):
    ...
    def __init__(self, m1, m2, name=None):
        ...
        self.m1 = m1
        self.m2 = m2
        if not (name is None or isinstance(name, str)) and isinstance(
            m1, SM) and isinstance(m2, SM):
            print m1, m2, name
    ...
    def done(self, state):
        (s1, s2) = state
        if not self.m1.done(s1):
            pass
        return self.m2.done(s2)
...
class Parallel(SM):
    ...
    def __init__(self, m1, m2, name=None):
        ...
        isinstance(m2, SM)
        self.name = name
        if not set(self.m1.legalInputs) == set(self.m2.legalInputs):
...
class Feedback(SM):
    ...
    def getNextValues(self, state, inp):
        ...
        if not o != 'undefined':
            raise AssertionError, 'Error in feedback; machine has no delay'
    ...
    def printDebugInfo(self, depth, state, nextState, inp, out, debugParams):
        ...
        self.m.printDebugInfo(depth + 4, state, nextState, lastOutput, out,
                              debugParams)
        self.doTraceTasks(inp, state, out, debugParams)
...
class FeedbackAdd(SM):
    ...
    def printDebugInfo(self, depth, state, nextState, inp, out, debugParams):
        if nextState and len(nextState) == 2:
            ...
            self.m1.printDebugInfo(depth + 4, s1, ns1, inp + o2, o1,
                                   debugParams)
...
class Parallel2(Parallel):
    ...
    def __init__(self, m1, m2):
        ...
        self.legalInputs = [
            (i1, i2)
            for i1 in self.m1.legalInputs for i2 in self.m2.legalInputs
        ]
...
class If(SM):
    ...
    def printDebugInfo(self, depth, state, nextState, inp, out, debugParams):
        if nextState and len(nextState) == 2:
            ...
            if ifState == 'runningM1':
                self.sm1.printDebugInfo(depth + 4, smState, nsmState, inp, out,
                                        debugParams)
            elif ifState == 'runningM2':
                self.sm2.printDebugInfo(depth + 4, smState, nsmState, inp, out,
                                        debugParams)
    ...
    def printDebugInfo(self, depth, state, nextState, inp, out, debugParams):
        if nextState and len(nextState) == 2:
            ...
            if machineRunning == 'M1':
                self.m1.printDebugInfo(depth + 4, s1, ns1, inp, out,
                                       debugParams)
            else:
                self.m2.printDebugInfo(depth + 4, s2, ns2, inp, out,
                                       debugParams)
            self.doTraceTasks(inp, state, out, debugParams)
...
class Sequence(SM):
    ...
    def __init__(self, smList, name=None):
        ...
        if not name is None:
            pass
        if not isinstance(name, str) or not isinstance(smList, (tuple, list)):
            raise Exception, 'Sequence takes a list of machines and an optional name argument'
        not isinstance(smList, (tuple, list))
        self.n = len(smList)
        self.name = name
...
def splitValue(v, n=2):
    ...
    if v == 'undefined':
        return ['undefined'] * n
    if not len(v) == n:
        raise AssertionError, 'Value wrong length'
    return v
...
class DebugParams:
    ...
    def __init__(self, traceTasks, verbose, compact, printInput):
        ...
        if not verbose:
            pass
        self.doDebugging = len(traceTasks) > 0
        self.k = 0
...
def allDefined(struct):
    ...
    if isinstance(struct, list) or isinstance(struct, tuple):
        return [](_[1], [allDefined(x) for x in struct])
    return True
