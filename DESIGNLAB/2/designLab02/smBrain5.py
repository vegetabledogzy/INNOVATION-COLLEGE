...
class MySMClass(sm.SM):
    ...
    def getNextValues(self, state, inp):
        ...
        if state=='go_straight':#开始行动
            if ((inp.sonars[2] < 0.25 and inp.sonars[5] < 0.25) or (inp.sonars[3] < 0.2 and inp.sonars[4] < 0.2) or inp.sonars[2] + inp.sonars[3] + inp.sonars[4] + inp.sonars[6] < 0.75):
                return ('conor', io.Action(fvel=-0.3, rvel=0.4))  # 墙角
            elif((inp.sonars[7]<0.3 or inp.sonars[6]<0.3) or (inp.sonars[0]<0.3 or inp.sonars[1]<0.3)):
        ...
        if state=='conor':
            ...
            elif((inp.sonars[7]<0.3 or inp.sonars[6]<0.3) and (inp.sonars[0]<0.3 or inp.sonars[1]<0.3)):
                return ('near',io.Action(fvel = 0.08,rvel = 0))
            else:
                return ('go_straight',io.Action(fvel = 0.1,rvel = 0))
