...
class MySMClass(sm.SM):
    def getNextValues(self, state, inp):
        near=0
         ...
         #       near=1
        if (inp.sonars[3]+inp.sonars[4])/2>0.5 and inp.sonars[0]>0.5 and inp.sonars[7]>0.5:
            near=0
        else:
            near=1
        ...
        if near ==1:
            if inp.sonars[0]+inp.sonars[1]+inp.sonars[2]+inp.sonars[3]<inp.sonars[4]+inp.sonars[5]+inp.sonars[6]+inp.sonars[7]:#在左墙边
                if inp.sonars[0]<0.4 or inp.sonars[1]<0.4:
                    if(inp.sonars[1]<0.5):
                ...
                if inp.sonars[7]<0.4 or inp.sonars[6]<0.4:
                    if(inp.sonars[6]<0.5):
                      return (state,io.Action(fvel = -0.08,rvel = 0.3))   
                    return (state,io.Action(fvel = 0.08,rvel = 0.3)) 
                else :
