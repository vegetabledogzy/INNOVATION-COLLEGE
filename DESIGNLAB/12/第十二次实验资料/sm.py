...
class SM():
    ...
    def start(self, traceTasks=[], verbose=False, compact=True, printInput=True):
        ...
        self.__debugParams = DebugParams(traceTasks, verbose, compact, printInput)
    ...
    def transduce(self, inps, verbose=False, traceTasks=[], compact=True, printInput=True, check=False):
        """
        ...
        n = len(inps)
        result = []
        self.start(verbose=verbose, compact=compact, printInput=printInput, traceTasks=traceTasks)
        if verbose:
    ...
    def run(self, n=10, verbose=False, traceTasks=[], compact=True, printInput=True, check=False):
        """
        ...
        @return: list of outputs
        """
        return self.transduce([None] * n, verbose=verbose, traceTasks=traceTasks, compact=compact, printInput=printInput, check=check)

    def transduceF(self, inpFn, n=10, verbose=False, traceTasks=[], compact=True, printInput=True):
        """
        Like C{transduce}, but rather than getting inputs from a list
        of values, get them by calling a function with the input index
        as the argument. 
        """
        return self.transduce([ inpFn(i) for i in range(n) ], traceTasks=traceTasks, compact=compact, printInput=printInput, verbose=verbose)
    ...
    def printDebugInfo(self, depth, state, nextState, inp, out, debugParams):
        ...
        if debugParams.verbose and not debugParams.compact:
            if debugParams.printInput:
                ...
                print util.prettyString(inp),
                print 'Out:', util.prettyString(out),
                print 'Next State:', util.prettyString(nextState)
            else:
...
class FeedbackAdd(SM):
    ...
    def startState(self):
        return (
         self.m1.getStartState(), self.m2.getStartState())
...
class Parallel2(Parallel):
    ...
    def __init__(self, m1, m2):
        ...
        self.legalInputs = [ (i1, i2) for i1 in self.m1.legalInputs for i2 in self.m2.legalInputs
                           ]
...
class If(SM):
    ...
    def getFirstRealState(self, inp):
        if self.condition(inp):
            ...
            return (
             'runningM2', self.sm2.getStartState())
    ...
    def printDebugInfo(self, depth, state, nextState, inp, out, debugParams):
        if nextState and len(nextState) == 2:
            ...
            if ifState == 'runningM1':
                self.sm1.printDebugInfo(depth + 4, smState, nsmState, inp, out, debugParams)
            elif ifState == 'runningM2':
                self.sm2.printDebugInfo(depth + 4, smState, nsmState, inp, out, debugParams)
            self.doTraceTasks(inp, state, out, debugParams)
