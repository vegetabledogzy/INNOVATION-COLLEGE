...
#module for serials IO for POSIX compatible systems, like Linux
#
...
####################################Imports#####################################
import sys, os, fcntl, cygwinreg, struct, select, errno
...
also add the device name of the serials port and where the
counting starts for the first serials port.
e.g. 'first serials port: /dev/ttyS0'
...
class Serial(SerialBase):
    ...
    def read(self, size=1):
        """Read size bytes from the serials port. If a timeout is set it may
    ...
    def write(self, data):
        """Output the given string over the serials port."""
        if self.fd is None: raise portNotOpenError
        t = len(data)
        d = data
        while t > 0:
    ...
    def fileno(self):
        """For easier of the serials port instance with select.
